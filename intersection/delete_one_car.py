...
def get_vehicles_from_movements (vehicles_data, moevment_car_id_ls, traget_lane_id_ls, traget_lane_points,
                                 driving_name=None, x_is_unixtime=False, output_points=False):
    lines_ls = []
    speed_ls = []
    if output_points:
        start_points = [[], []]
        ...
        for veh_id, veh_data in vehicles_data.items ():
            ...
            if car_id_movement == veh_id:
                ...
                for i in range (len (lane_id) - 1):
                    ...
                    if speed > -1 and lane_id [i] in traget_lane_id_ls:
...
def run_main (pkl_file, movement_file):
    ...
    # print(movement_dict)
    movement_car_id_ls = movement_dict ['straight_1_to_6']
    traget_lane_id_ls = [1, 50, 6]
    traget_lane_points = [1, 6]
    lines_ls, speed_ls, points = get_vehicles_from_movements (vehicle_data, movement_car_id_ls, traget_lane_id_ls,
                                                              traget_lane_points, output_points = True)
    fig_path = '/data3/liyitong/HuRong_process/D2_straight_1_to_6.jpg'
